        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="card-header align-items-center d-flex">
                    <h4 class="card-title mb-0 flex-grow-1">{{ isset($about) ? 'Edit' : 'Add' }} about</h4>
                </div><!-- end card header -->
                <div class="card-body">
                    <div class="live-preview">
                        <form class="row g-3 needs-validation ajaxForm" novalidate action="{{ route('about.save') }}"
                            method="post" enctype="multipart/form-data">
                            @csrf

                           @if($about->colum5 == null)
                            
                            <div class="col-xxl-3 col-md-6">
                                <label for="" class="form-label">Content</label>
                                <textarea class="form-control" name="colum1" class="form-control form-control-icon" placeholder="Content" cols="30"
                                rows="10">{{ $about->colum1 ?? '' }}</textarea>
                            </div> 
                            @endif
                            <div class="col-xxl-3 col-md-6">
                                <label for="" class="form-label">Content</label>
                                <textarea type="text" class="form-control" name="colum2" class="form-control form-control-icon" placeholder="Content" cols="30"
                                rows="10">{{ $about->colum2 ?? '' }}</textarea>
                            </div>
                           @if($about->colum5 !== null)
                            <div class="col-xxl-3 col-md-6">
                            ...
                            <div class="col-xxl-6 col-md-6">
                                <label for="iconInput" class="form-label">Content </label>
                                <textarea name="colum3" class="form-control form-control-icon" placeholder="Content" cols="30"
                                rows="5">{{ $about->colum3 ?? '' }}</textarea>
                            </div>
                            @if($about->colum5 !== null)
                                <div class="col-xxl-6 col-md-6">
                                    <img src="{{ asset('images/'.$about->colum1) }}" alt="about Image" width="100%" height="300px" class="me-1">
                                </div>
                            @endif
                            <div class="col-xxl-6 col-md-6">
                                <label for="iconInput" class="form-label">Content </label>
                                <textarea name="colum4" class="form-control form-control-icon" placeholder="Content" cols="30"
                                    rows="10">{{ $about->colum4 ?? '' }}</textarea>
                            </div>
                            @if($about->colum5 !== null)
                            ...
                            @endisset
                            <div class="col-12">
                                <button class="btn btn-primary" type="submit"> {{ isset($about) ? 'Edit' : 'Add' }}
                                    about</button>
                            </div>
                        </form>
                    </div>
